...
class GraphQueryManager:
    ...
    def find_path_between_nodes(self, source_id: str, target_id: str, max_length: int = 5) -> Dict[str, Any]:
...
# FASTAPI APP
# --------------------------------------------------------------------------
app = FastAPI(title="Neo4j Graph Service", lifespan=lifespan, version="1.0.0")

@app.get("/")
async def root():
    return {
        "service": "Neo4j Graph Service",
        "version": "1.0.0",
        "status": "running",
...
async def query_graph(request: GraphQueryRequest, http_request: Request):
    ...
    try:
        driver = http_request.app.state.driver
        query_manager = GraphQueryManager(driver)
        results = []
...
async def batch_neighbor_search(request: NeighborSearchRequest, http_request: Request):
...
@app.post("/path")
async def find_path(request: PathQueryRequest, http_request: Request):
...
# --------------------------------------------------------------------------
@app.get("/stats")
async def get_graph_statistics(http_request: Request):
...
async def get_central_nodes(limit: int = 10, http_request: Request = None):
...
async def get_node_details(node_id: str, http_request: Request):
    """Get detailed information about a specific node"""
    try:
        driver = http_request.app.state.driver
        query_manager = GraphQueryManager(driver)
